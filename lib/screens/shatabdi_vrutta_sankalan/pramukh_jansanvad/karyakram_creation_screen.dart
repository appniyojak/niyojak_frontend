...
class KaryakramCreationScreen extends StatefulWidget {
  static const routeName = '/karyakram-creation-screen';

  const KaryakramCreationScreen({super.key});

  @override
  State<KaryakramCreationScreen> createState() => _KaryakramCreationScreenState();
}

class _KaryakramCreationScreenState extends State<KaryakramCreationScreen> {
  ...
  Future<void> populateAllDropdowns(int level, DropDownModel dm) async {
    setState(() {
      // _selctedLevelList = [];
      _selectedGeoUnitIdForCreat =
          _linkedMahaanagarValue = _linkedbhaagValue = _linkedshaharValue = _linkedupnagarValue = _linkednagarValue = _linkedmandalValue = _linkedgraamValue = _linkedvastiValue = null;
    });
  ...
  createSadbhavBaithakFun() async {
    ...
    log("Form Data (JSON):\n$formattedJson");
    final _res = await Statics.CreateUpdatePramukhJanData(context: context, inputJson: formData, showLoader: true);
    if (_res) {
