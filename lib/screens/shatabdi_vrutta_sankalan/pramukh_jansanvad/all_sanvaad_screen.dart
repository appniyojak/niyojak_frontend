...
class AllSanvaadScreen extends StatefulWidget {
  static const String routeName = '/all-pramukh_jansanvad-list-view';

  const AllSanvaadScreen({super.key});

  @override
  State<AllSanvaadScreen> createState() => _AllSanvaadScreenState();
}

class _AllSanvaadScreenState extends State<AllSanvaadScreen> {
  ...
  getAllData() async {
    ...
    final _data = await Statics.GetAllPramukhJanKaryakramListData(context: context, inputJson: formData, showLoader: true);
  ...
  deleteSadbhavBaithakFun(String id) async {
    ...
    await Statics.DeletePramukhJanData(context: context, inputJson: formData, showLoader: true) ?? [];
...
// print("populatelinkedNagarDropdown ${bhaagIDStr} == ${shaharIDStr}  ");
  ...
  List<String> headers = ["serialNo", "LevelName", "sanvaadKaryakram", "date2", "anya_upastiti_male", "anya_upastiti_matrushakti", "Total"];
